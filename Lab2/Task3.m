...
% Last Modified by GUIDE v2.5 28-Mar-2021 15:41:53
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Y, Fs]=audioread('meRecording.wav');
...
[a(1),b(1)]=Filters(Fs);
[a(2),b(2)]=filter_4kto8k(Fs);
[a(3),b(3)]=filter_9kto13k(Fs);
[a(4),b(4)]=filter_13kto17k(Fs);
[a(5),b(5)]=filter_high(Fs);
for num = 1:length(slideValue)
    y=y+filter(10^(sliderValue(num)/20)*b(num),a(num),audioR);
    if num>=length(slideValue)
        disp('Hello There');
        setappdata(0,'finalData',y);
    end
end
t = length(y)./fs;
plot(t,y);
